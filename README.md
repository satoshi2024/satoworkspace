        ...
        try:
            ...
            for b_key in wrt_dict.keys():
                ...
                if b_key in send_de_map:
                    # 找到原始完整行
                    old_row_df = df_send[df_send[1].astype(str).str.strip() == b_key]
                    if not old_row_df.empty:
                        full_old_row = old_row_df.iloc[0].tolist()
                        old_c = str(full_old_row[2]).strip() if len(full_old_row) > 2 and pd.notna(full_old_row[2]) else ""
                    carried_count += 1
                else:
                    new_count += 1
                ...
                if full_old_row is not None:
                    # 保留原始行的所有列，只更新前5列
                    new_row = full_old_row.copy()
                    new_row[0] = wrt_row['A']
                    new_row[1] = wrt_row['B']
                    new_row[2] = final_c
                    # D 和 E 由第二阶段 parser 更新，这里先保留
                    new_data.append(new_row)
                else:
                    # 新增行：创建与原始列数相同的行，前5列有值，其余为空
                    new_row = [""] * n_cols
                    new_row[0] = wrt_row['A']
                    new_row[1] = wrt_row['B']
                    new_row[2] = final_c
            ...
            self.log(f"重建完成：从 WRT 同步 {len(df_new)} 行（保留原始 SEND 的 {n_cols} 列结构）")
